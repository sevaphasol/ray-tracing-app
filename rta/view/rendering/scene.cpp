...
rta::view::Scene::Scene( hui::WindowManager*         wm,
                         const dr4::Vec2f&           pos,
      ...
      background_color_( background_color )
{
    ...
    model_.addSphere( model::Material( rta::model::Color( 8, 32, 8 ), 0.3f ),
                      rta::model::Vector3f( 2, 2, -13 ),
                      1.5 );

    model_.addSphere( model::Material( rta::model::Color( 118, 185, 0 ), 0.9f ),
                      rta::model::Vector3f( 0, 3, -16 ),
                      1.5 );

    model_.addSphere( model::Material( rta::model::Color( 118, 185, 0 ), 0.0, 0.7f, 0.8f ),
                      rta::model::Vector3f( -1.5, -0.5, -13.6 ),
                      1.5 );
...
void
rta::view::Scene::setOnSelectionChanged( rta::view::Scene::SelectionChangedCb cb )
{
    on_selection_changed_ = std::move( cb );
}

rta::model::SceneManager&
rta::view::Scene::getModel()
{
    need_update_ = true;
...
bool
rta::view::Scene::onIdle( const hui::Event& event )
{
    if ( !visible_ )
    {
        return false;
...
bool
rta::view::Scene::onMousePress( const hui::Event& event )
{
    if ( !visible_ )
    {
        return false;
    ...
    for ( size_t t = 0; t < num_threads; ++t )
    {
        ...
        workers.emplace_back( [this, w, start_row, end_row]() {
            for ( size_t row = start_row; row < end_row; ++row )
            {
                ...
                {
                    rta::model::Color color = model_.calcPixelColor( row,
                                                                     col,
                                                                     { background_color_.r,
                                                                       background_color_.g,
                                                                       background_color_.b,
...
void
rta::view::Scene::setCameraPosString()
{
...
void
rta::view::Scene::RedrawMyTexture() const
{
    if ( !visible_ )
    {
        return;
...
bool
rta::view::Scene::isVisible() const
{
    return visible_;
}

std::optional<size_t>
rta::view::Scene::findIndexForObj( model::Primitive* obj ) const
{
